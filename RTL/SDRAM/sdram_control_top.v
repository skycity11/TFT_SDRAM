...
`include        "../RTL/SDRAM/Sdram_Params.h"
   ...
   output[7:0]		     Rd_use;		   //读FIFO数据可用数据量
	...
	output[`ASIZE-1:0]  sdram_addr;             //SDRAM地址总线 
	output[`BSIZE-1:0]  sdram_ba;             //SDRAMBank地址 
	output              sdram_cs_n;           //SDRAM片选信号 
	output              sdram_cke;            //SDRAM时钟使能
	output              sdram_ras_n;          //SDRAM行地址选
	output              sdram_cas_n;          //SDRAM列地址选
	output              sdram_we_n;           //SDRAM写使能
	inout [`DSIZE-1:0]  sdram_dq;             //SDRAM数据总线 
	output[`DSIZE/8-1:0]sdram_dqm;            //SDRAM数据掩码
